...
@click.command(
    help="To create the app, you need to navigate to the you_project directory in the project, and then run it.")
@click.argument('app_name')
def main(app_name: str):
    target_dir = Path.cwd() / 'src' / 'api'
    package_dir = Path(__file__).parent
    api_dir = target_dir
    if not api_dir.exists():
        click.echo('Error: You must create a project first before creating an app.', err=True)
